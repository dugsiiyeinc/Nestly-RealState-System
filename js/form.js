...
authForm?.addEventListener("submit", (e) => {
  ...
  if (signIn) {
    ...
    const existingUser = users.find(
      (user) => user.password == password.value && user.email == email.value
    );
    if (existingUser) {
...
contactForm.addEventListener("submit", (e) => {
  ...
  let existContactUser = oldContact.find(
    (CoUser) => CoUser.email == email.value && CoUser.number == number.value
  );
